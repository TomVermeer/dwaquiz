...
export const setParticipatingTeams = (teamNames) => {
  return {
    type: SharedActions.ON_TEAM_APPROVAL,
    payload: teamNames,
  };
};
...
export const fetchParticipatingTeams = (quizPin) => (dispatch) => {
  getAndParse(`quiz-nights/${quizPin}/teams`).then((json) =>
    dispatch(setParticipatingTeams(json))
  );
};

export const fetchAnsweredTeams = (quizPin, roundNumber, questionNumber) => (
  dispatch
) => {
  getAndParse(
    `quiz-nights/${quizPin}/rounds/${roundNumber}/questionings/${questionNumber}/answers`
